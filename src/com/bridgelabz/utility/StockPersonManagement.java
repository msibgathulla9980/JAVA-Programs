...
public class StockPersonManagement {
	...
	public void createPersonalAccount() throws IOException {
		System.out.println("Enter the account name to be created (eg:file.json)");
		...
		if (rs) {
			...
			System.out.println("The Entered Account name already exists");
		}
	}

	public void openPersonnelAccount() throws JsonParseException, JsonMappingException, IOException {
		System.out.println("Enter your account name with extension (.json) to open");
		...
		for (File file : arrayOfFiles) {
			...
			if (account.equals(filename)) {       	
				if (file.length() >0) {
					...
					System.out.println("Do you want to add a new data onto your Account?");
					insertDetails();
				}
				flag = 1;
			}
	...
	public void insertDetails() throws JsonGenerationException, JsonMappingException, IOException {
		StockPersonOperation stockPersonOperation = new StockPersonOperation();
		boolean run = true;
		while (run == true) {
			System.out.println("1.Buy Stock\n2.Sell Stock\n3.Save\n4.Display\n5.Display Using Data Structures\n6.Go back to main menu");
			...
			case 3:
				...
				if (flag == 0) {
					System.out.println("File doesnot exist or you have not given a correct extention(.json)");
				}
			...
			run = true;
			break;
			case 5:stockPersonOperation.displayDynamic();
			run = true;
			break;
